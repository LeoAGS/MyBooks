            ...
            foreach (var work in works)
            {
                foreach (var copy in work.Copies.OrderBy(copy => copy.Format).ThenBy(copy => copy.Location))
                {
                    AppendCsvRow(csv, [
                        ...
                        work.Author,
                        copy.CopyTitle,
                        string.Join("; ", copy.ContainedWorks.OrderBy(link => link.SortOrder).Where(link => link.Work is not null).Select(link => link.Work!.Title)),
                        work.Genre,
                        work.Category,
                        work.CollectionName,
                        ...
                        copy.VolumeCount.ToString(CultureInfo.InvariantCulture),
                        copy.Condition,
                        copy.Location,
